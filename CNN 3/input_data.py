...
#    img_depth = 3
    ...
    with tf.name_scope('input'):
        ...
        image_list=[]
        galaxyid_list=list(index1)

        for i in range(len(index1)):
            image_list.append(data_dir+str(index1[i])+'.jpg')

        temp = np.array([image_list, label_list,galaxyid_list])
        temp = temp.transpose()
        # np.random.shuffle(temp)
    
        image_list = list(temp[:, 0])
        label_list = list(temp[:, 1])
        galaxyid_list=list(temp[:, 2])
        label_list = [round(float(i)) for i in label_list]
        galaxyid_list=[round(float(i)) for i in galaxyid_list]
        
        image = tf.cast(image_list, tf.string)
        label = tf.cast(label_list, tf.int32)
        galaxyid=tf.cast(galaxyid_list, tf.int32)
        # make an input queue
        input_queue = tf.train.slice_input_producer([image, label,galaxyid])
    
        label = input_queue[1]
        label = tf.cast(label, tf.int32)
        galaxyid = input_queue[2]
        galaxyid = tf.cast(galaxyid, tf.int32)
...
def read_galaxy11(data_dir,label_dir,batch_size):
    ...
    image,label,galaxyid=get_images(data_dir,label_dir)
...
#    image = tf.image.random_hue(image, max_delta=0.2)
    ...
    images, label_batch,galaxyid_batch = tf.train.shuffle_batch(
                                         [image, label,galaxyid], 
                                         batch_size = batch_size,
                                         num_threads= 64,
                                         capacity = 20000,
                                         min_after_dequeue = 3000)
    images = tf.cast(images, tf.float32)
    galaxyid_batch=tf.cast(galaxyid_batch,tf.int32 )    
    ## ONE-HOT      
    n_classes = 5
    ...
    return images, label_batch, galaxyid_batch
#%%
...
def read_galaxy11_test(data_dir,label_dir, batch_size):
    ...
    """
    image,label,galaxyid=get_images_test(data_dir,label_dir)
    ...
    images, label_batch,galaxyid_batch  = tf.train.batch(
                                         [image, label,galaxyid],
                                         batch_size= batch_size,
                                         num_threads = 64,
                                         capacity= 2000)
